module VoicemailHelper
	def voicemail_records(call_recordings)
	  	content_tag :section, class: 'recordingsbodytable' do
	      call_recordings.present? ? voicemail_recording_inner_records(call_recordings) : voicemail_recordings_no_records
	    end
	end
	def voicemail_recording_inner_records(call_recordings)
    call_recordings.map do |call_recording|
      concat content_tag(:section, class: 'table_row') {
        ...
        content_tag(:div, (Recording.voicemail_name(call_recording.id)), class: 'phoneScriptCall')
      }
    end
  end

  def voicemail_recordings_no_records
    concat content_tag(:section, class: 'table_row') {
      content_tag(:div, class: 'threeColum') {
